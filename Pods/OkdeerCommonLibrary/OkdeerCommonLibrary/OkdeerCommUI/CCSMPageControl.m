...
#define DEFAULT_INDICATOR_MARGIN 5.0f

@interface CCSMPageControl ()
@end

@implementation CCSMPageControl
{
@private
...
- (void)_renderPages:(CGContextRef)context rect:(CGRect)rect
{
	...
	for (NSUInteger i = 0; i < _numberOfPages; i++) {
		...
		if (image) {
            ...
            [image drawInRect:CGRectMake(xOffset, yOffset, _measuredIndicatorWidth, _measuredIndicatorHeight)];
		} else {
...
- (void)sizeToFit
{
	...
	CGSize size = [self sizeForNumberOfPages:self.numberOfPages];
	size.height = MAX(size.height, 12.0f);
//    size.height = 10 ;
	frame.size = size;
	self.frame = frame;
}
...
- (void)_updateMeasuredIndicatorSizes
{
	...
	if (self.pageIndicatorImage) {
		...
		_measuredIndicatorWidth = MIN(_indicatorDiameter, imageSize.width);
		_measuredIndicatorHeight = MIN(_indicatorDiameter, imageSize.height );
//        _measuredIndicatorWidth = 8;
//        _measuredIndicatorHeight = 8;
	}
